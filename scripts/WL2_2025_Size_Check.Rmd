...
dat3$sheets[[12]] %>% rowwise() %>% filter(!is.na(overhd.perp)) %>%  filter(is.na(as.numeric(overhd.perp))) #MA instead of NA
dat3$sheets[[12]] %>% rowwise() %>% filter(!is.na(herbv.scale)) %>%  filter(is.na(as.numeric(herbv.scale))) #Ns - should check 

dat3$sheets[[13]] %>% rowwise() %>% filter(!is.na(row)) %>%  filter(is.na(as.numeric(row))) #header row 
```
