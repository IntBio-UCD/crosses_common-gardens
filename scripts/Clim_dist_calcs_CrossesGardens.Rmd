...
# Code for calculating climate distance
...
## Load Climate Data
```{r}
all_clim <- read_csv("../output/Climate/WtrYr_Clim_2025TradWtrYr.csv")
```
...
#Oct 2024-Sept 2025
wl2_garden_2025_wtryr <- all_clim %>% 
  filter(parent.pop=="WL2_Garden", year==2025) %>%
  rename_with(.cols = c(8:22), ~paste0(.x, '_Garden')) %>% 
  select(cwd_Garden:ppt_coldest_month_Garden)
```
...
recent_wtryr_clim <- all_clim %>% 
  filter(timeframe=="recent", year!=2025, #30 year averages start from 2024 back
         Season=="Water Year", 
         !str_detect(parent.pop, "Garden"))
...
historic_wtryr_clim <- all_clim %>% 
  filter(timeframe=="historic", year!=2025,
         Season=="Water Year", !str_detect(parent.pop, "Garden"))
...
wl2_garden_2025_wtryr_range_prep <- wl2_garden_2025_wtryr %>% 
  rename_with(~str_remove(., "_Garden"), everything())
...
all_WL2_gowers <- bind_rows(boot_gowers_recent_wtryr_pops_2024_WL2,
  ...
  mutate(WL2_Lat=38.82599, WL2_Long=-120.2509) %>% 
...
all_UCD_gowers <- bind_rows(boot_gowers_recent_wtryr_pops_2024_UCD,
  ...
  mutate(UCD_Lat=38.53250, UCD_Long=-121.78299) %>% 
...
```{r}
# water year - recent
gowers.boot_2025_WL2_recent_wtryr <- boot(data=recent_wtryr_clim_nest, statistic=gowers_calc, R=1000, P=15, 
                                      garden_clim=wl2_garden_2025_wtryr, 
                                      garden_range=wl2_garden_2025_wtryr_range_prep) #will sample each row (year) with replacement 

for(i in 1:10) {
  plot(gowers.boot_2025_WL2_recent_wtryr, index=i) #distributions look normal for the most part 
}

boot_recent_wtryr_results_2025_WL2 <- tidy(gowers.boot_2025_WL2_recent_wtryr,conf.int=TRUE,conf.method="norm") %>%  #all pops
  rename(Gowers_Dist = statistic) %>% 
  mutate(timeframe="Recent", Season="Water Year", Year=2025)
boot_gowers_recent_wtryr_pops_2025_WL2 <- bind_cols(pops, boot_recent_wtryr_results_2025_WL2) %>% 
  select(parent.pop:Long, timeframe, Season, Year, Gowers_Dist, conf.low, conf.high) %>% 
  arrange(Gowers_Dist)

# water year - historic
gowers.boot_2025_WL2_historic_wtryr <- boot(data=historic_wtryr_clim_nest, statistic=gowers_calc, R=1000, P=15, garden_clim=wl2_garden_2025_wtryr, garden_range=wl2_garden_2025_wtryr_range_prep) #will sample each row (year) with replacement 

for(i in 1:10) {
  plot(gowers.boot_2025_WL2_historic_wtryr, index=i) #distributions look normal for the most part 
}

boot_historic_wtryr_results_2025_WL2 <- tidy(gowers.boot_2025_WL2_historic_wtryr,conf.int=TRUE,conf.method="norm") %>%  #all pops
  rename(Gowers_Dist = statistic) %>% 
  mutate(timeframe="Historic", Season="Water Year", Year=2025)
boot_gowers_historic_wtryr_pops_2025_WL2 <- bind_cols(pops, boot_historic_wtryr_results_2025_WL2) %>% 
  select(parent.pop:Long, timeframe, Season, Year, Gowers_Dist, conf.low, conf.high) %>% 
  arrange(Gowers_Dist)

all_WL2_gowers_2025 <- bind_rows(boot_gowers_recent_wtryr_pops_2025_WL2,
                            boot_gowers_historic_wtryr_pops_2025_WL2) %>% 
  mutate(WL2_Lat=38.82599, WL2_Long=-120.2509) %>% 
  mutate(Geographic_Dist=distHaversine(cbind(WL2_Long, WL2_Lat), cbind(Long, Lat)))
write_csv(all_WL2_gowers_2025, "../output/Climate/WL2_2025_Clim_Dist.csv")
```
...
all_WL2_gowers_2025 %>% 
  ...
  theme(text=element_text(size=25), axis.text.x = element_text(angle = 45,  hjust = 1)) +
  facet_wrap(~timeframe)
ggsave("../output/Climate/wtryear_2025Gowers_fromWL2.png", width = 24, height = 8, units = "in")
