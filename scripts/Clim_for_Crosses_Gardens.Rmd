...
flint_all_year_wtr_yr <- flint_all_year_mosnums %>% 
  ...
  mutate(timeframe = case_when(
                                ...
                                wtr_yr >= 1995 & wtr_yr <= 2024 ~ "recent")) %>%
...
wtr_yr_clim <- left_join(flint_yrly_avgs_wtr_yr, bioclim_wtryr_to_merge) %>% 
  ...
  filter(year<2025) #remove Nov-Dec 2024 b/c that would be the start of the 2025 water year 
write_csv(wtr_yr_clim, "../output/Climate/WtrYr_Clim.csv")
```
