...
wl2_2024_est %>% filter(Pop.Type=="Parent") %>% select(pop) %>% distinct() #10 parent pops with establishment (only planted 10 starting this year)
...
#wl2_2025_est %>% filter(Pop.Type=="Parent") %>% select(pop.id) %>% distinct() #10 parent pops with establishment
...
#ucd_2024_est %>% filter(str_detect(`Plant Type`, "Parent")) %>% select(pop.id) %>% distinct() #10 parent pops with establishment
```
