...
# Annual Census
Need to remove the google sheets. 
```{r}
#load the libraries
library(tidyverse)
#tutorial at https://bookdown.org/ansellbr/WEHI_tidyR_course_book/week-4-part-3.html
data_files <- list.files('~/bmquarles@ucdavis.edu - Google Drive/Shared drives/IntBioTeam/Common Gardens/WL2_2024/CorrectedCSVs',pattern='annual_census',ignore.case=TRUE)
...
for (i in data_names) {
  ...
  #check for spaces in numbers
  data2 <- data %>% filter(grepl(" ", height.cm)) %>% mutate(problem="spaces")
  
  #check for extreme values
  data3 <- data %>% mutate(height.cm=as.double(height.cm))
  data3 <- data3 %>% filter(!is.na(block)) %>% filter(height.cm>15) %>% mutate(problem="extreme value")
