...
by_pop_parents_Surv %>% 
  ...
  geom_errorbar(aes(ymin=mean_Surv-sem_Surv,ymax=mean_Surv+sem_Surv),width=.2, position = 
                  position_dodge(0.75)) +
  labs(x="Parent Population", y="Year 1 Survival", fill="Elevation (m)", title="2024 Parents") +
  theme_classic() + 
...
by_pop_Surv_F1 <- by_pop_Surv %>% 
  ...
  left_join(elev_info, by=join_by(dame_pop==pop.id)) %>% 
  ...
  left_join(elev_info, by=join_by(sire_pop==pop.id)) %>% 
  select(-elevation.group, -Season, -Year) %>% 
