...
dat5 %>% filter(bed=="B", row=="12", col=="A")
dat5 %>% filter(unique.ID=="1093") %>% arrange(censusN)#resprout
dat5 %>% filter(unique.ID=="160") %>% arrange(censusN) #no notes, but it has the wrong bed, row col on Census15 (B6A instead of D34A) 
dat5 %>% filter(unique.ID=="196") %>% arrange(censusN) #noted as missing for census 12
dat5 %>% filter(unique.ID=="2038") %>% arrange(censusN) #in two locations for census 12 (B12A, correct is A21C) 
dat5 %>% filter(unique.ID=="535") %>% arrange(censusN) #notes say "can't reach for census 15
