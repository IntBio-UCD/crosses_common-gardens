...
f1_surv_summary <- f1_surv %>% 
  group_by(pop,Pop.Type, dame_pop, sire_pop, dame_elev, dame_GD_Recent, dame_GD_Historic, meanElev, mean_GD_Recent, mean_GD_Historic) %>% 
...
## Compare F1s to Mid-Parent Value 
```{r}
unique(f1_surv$pop)

parent_surv_summary_prep <- parent_surv_summary %>% 
  ungroup() %>% 
  select(pop, mean_Surv) %>% 
  pivot_wider(names_from = pop, values_from = mean_Surv) %>% 
  mutate("WV x TM2"=(WV+TM2)/2, "LV1 x WL2"=(LV1+WL2)/2,
         "LV1 x TM2"=(LV1+TM2)/2, "TM2 x WL2"=(TM2+WL2)/2,
         "CC x TM2"=(CC+TM2)/2, "WV x WL2"=(WV+WL2)/2,
         "BH x TM2"=(BH+TM2)/2, "WL2 x BH"=(BH+WL2)/2,
         "DPR x WL2"=(DPR+WL2)/2, "BH x WL2"=(BH+WL2)/2,
         "SQ3 x WL2"=(SQ3+WL2)/2, "TM2 x YO11"=(TM2+YO11)/2) %>% 
  pivot_longer(cols = everything(), names_to = "pop", values_to = "mean_Surv") %>% 
  filter(str_detect(pop, "x")) %>% 
  mutate(pop = paste0(pop, "_midParent"))

f1_surv_summary %>% 
  bind_rows(parent_surv_summary_prep) %>% 
  ggplot(aes(x=pop, y=mean_Surv, fill=meanElev)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) + 
  geom_errorbar(aes(ymin=mean_Surv-sem_surv,ymax=mean_Surv+sem_surv),width=.2, position = 
                  position_dodge(0.75)) +
  theme_classic() + 
