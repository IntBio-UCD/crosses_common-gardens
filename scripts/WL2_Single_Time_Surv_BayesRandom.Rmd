...
```{r}
surv_parent_binary_bf1 <- brmsformula(Surv_to_Oct ~ (1|WL2)+(1|BH)+(1|WV)+(1|LV1)+(1|TM2)+(1|SQ3)+(1|DPR))

get_prior(surv_parent_binary_bf1, family = "bernoulli", data = wl2_surv_F1_binary)
```

```{r}
surv_parent_binary_m1 <- brm(surv_parent_binary_bf1, 
                             family = "bernoulli",
                             data = wl2_surv_F1_binary,
                             cores=4,
